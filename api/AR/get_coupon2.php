...
//
	if (($member_id != '') && ($member_pwd != '') && ($shopping_area != '')) {
		...
		try {
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					$sql2 = "SELECT * FROM mycoupon where mycoupon_trash=0 and mid=$mid  ";
					...
					if ($result2 = mysqli_query($link, $sql2)){
						if (mysqli_num_rows($result2) == 0){
							...
							try {
								...
								$sql = $sql." select $mid,'$coupon_no',cid,coupon_id ,coupon_name, coupon_type, coupon_description, coupon_startdate, coupon_enddate, coupon_status, coupon_rule, coupon_discount, discount_amount, coupon_storeid, '$shopping_area', coupon_picture from coupon where cid > 0 ";
