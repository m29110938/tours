...
//reserve_note
$reserve_note = isset($_POST['reserve_note']) ? $_POST['reserve_note'] : '';

	if (($member_id != '') && ($member_pwd != '') && ($pid != '') && ($reserve_date != '') && ($reserve_time != '') && ($program_person != '')) {
		//check 帳號/密碼
		$host = 'localhost';
		$user = 'tours_user';
		$passwd = 'tours0115';
		...
		try {
			...
			$reserve_note  = mysqli_real_escape_string($link,$reserve_note);
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					// add reserveinfo
					$sql="INSERT INTO class_reserveinfo (booking_no,store_id,pid, reserve_date, reserve_time, mid,member_id, member_name,member_email,program_person,reserve_note,reserve_remark, reserve_status, reserve_created_at) VALUES";
					$sql=$sql."('$booking_no',$sid, $pid, '$reserve_date', '$reserve_time', $mid, '$member_id', '$member_name','$member_email', $program_person,'$reserve_note','$reserve_remark', 0, NOW());";
