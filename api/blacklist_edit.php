					...
					try {
						...
						if ($result = mysqli_query($link, $sql)){
							if (mysqli_num_rows($result) > 0){
								...
								$sql=$sql."where store_id=$membersid and member_id=$m_id and blacklist_status=0 ;";
								...
								$data["status"]="true";
								$data["code"]="0x0200";
								$data["responseMessage"]="The member has been removed from the blacklist of this store!";	
							}else{
								$data["status"]="false";
								$data["code"]="0x0201";
