			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					try {
						...
						//$sql2 = $sql2." and reserve_status=0 and booking_no='$booking_no' and mid=".$mid;
						...
						if ($result2 = mysqli_query($link, $sql2)){
							if (mysqli_num_rows($result2) > 0){
								...
								$sql3="update reserveinfo set reserve_status=2 where rid=".$rid;
								mysqli_query($link,$sql3) or die(mysqli_error($link));

								$data["status"]="true";
								$data["code"]="0x0200";
								$data["responseMessage"]="The booking information have been canceled!";
							}else{
								$data["status"]="false";
								$data["code"]="0x0201";
