...
if (($member_id != '') && ($member_pwd != '')) {
	...
	if ($result = mysqli_query($link, $sql)){
		if (mysqli_num_rows($result)>0){
			...
			if (($mobile_no != '') && ($FCM_title != '') && ($FCM_content != '') ) {
				//&& ($Person_id != '') 
				try {
					...
					$sql = "SELECT * FROM member where member_trash=0 ";   //and member_status='1' 
					if ($mobile_no != "") {	
						$sql = $sql." and member_id='".$mobile_no."'";
					}
					...
					if ($result = mysqli_query($link, $sql)){
						if (mysqli_num_rows($result)>0){
							while($row = mysqli_fetch_array($result)){
								...
								$sql = "INSERT INTO notificationlog (Person_id, Role, msg, fcmresult, updatetime) VALUES ('$mobile_no', '0', '$msg', '$fcmresult', NOW())";
