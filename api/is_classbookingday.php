...
//
	if (($member_id != '') && ($member_pwd != '') && ($pid != '') && ($reserve_date != '' )) {
		//check 帳號/密碼
		$host = 'localhost';
		$user = 'tours_user';
		$passwd = 'tours0115';
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					try {
						...
						$sql2 = "SELECT * FROM class_reserveinfo ";
			
						$sql2 = $sql2." where rid > 0 and reserve_trash=0 ";
						if ($pid != "") {	
							$sql2 = $sql2." and pid=".$pid."";
						}
						if ($reserve_date != "") {	
...
//exit;				
						...
						if ($result2 = mysqli_query($link, $sql2)){
							if (mysqli_num_rows($result2) > 0){
								...
								$data["status"]="false";
								$data["code"]="0x0201";
								$data["responseMessage"]="No program has been booking for this day!";	
							}
						}else{
							$data["status"]="false";
							$data["code"]="0x0204";
