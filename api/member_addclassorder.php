			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					if ($result2 = mysqli_query($link, $sql2)){
						...
						if ($booking_no !=""){
								$sql3="update class_reserveinfo set reserve_status=1 where reserve_status=0 and mid=$mid and store_id=$membersid and booking_no='".$booking_no."'";
								mysqli_query($link,$sql3) or die(mysqli_error($link));
						}
												...
												$data["status"]="true";
												$data["code"]="0x0200";
												$data["responseMessage"]="The class order data is successfully added!";	
												
											}else{
												$data["status"]="false";
												$data["code"]="0x0206";
												$data["responseMessage"]="Add class order fail!";								
											}
										} catch (Exception $e) {
