...
//add_order(member_id,store_id,coupon_no,order_amount,discount_amount,pay_type,order_pay,order_status
	...
	if (($member_id != '') && ($member_pwd != '') && ($sid != '') && ($order_amount != '0')) {
		//check 帳號/密碼
		$host = 'localhost';
		$user = 'tours_user';
		$passwd = 'tours0115';
		...
		try {
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					$sql2 = "SELECT * FROM store where store_trash=0 ";
					if ($sid != "") {	
					...
					if ($result2 = mysqli_query($link, $sql2)){
									...
									if ($coupon_storeid == $membersid){
										...
										try {
											...
											if ($rvalue > 0) {
												...
												// $booking_no 預約號碼
												if ($booking_no !=""){
														$sql3="update reserveinfo set reserve_status=1 where reserve_status=0 and mid=$mid and store_id=$membersid and booking_no='".$booking_no."'";
														mysqli_query($link,$sql3) or die(mysqli_error($link));
												}
						...
						// 沒使用coupon
														...
														if (mysqli_num_rows($result5) > 0){
															...
															while($row5 = mysqli_fetch_array($result5)){
																...
																if ($store_service == '1') {
																	...
																	// $date1 = $date2->modify('+14 day');
																	// //echo $date1->format('Y-m-d') . "\n";	
																	// $sql3="update orderinfo set urate=$user_rate ,bonus_get=$bonus,bonus_date='".$date1->format('Y-m-d')." 00:04:00' where order_no='".$order_no."'";
																	// //echo $sql3;
							...
							$data["status"]="false";
							$data["code"]="0x0207";
							$data["responseMessage"]="The store id is wrong!";							
						}
					}else{
						$data["status"]="false";
						$data["code"]="0x0204";
