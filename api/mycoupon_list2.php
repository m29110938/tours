...
$using_flag = isset($_POST['using_flag']) ? $_POST['using_flag'] : '';
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					try {
						...
						//exit;
						//$sql2 = "SELECT pid,mid, coupon_no, using_flag, using_date, coupon_id ,coupon_name, coupon_type, coupon_description, DATE_FORMAT(coupon_startdate, '%Y-%m-%d') as coupon_startdate,DATE_FORMAT(coupon_enddate, '%Y-%m-%d') as coupon_enddate, coupon_status, coupon_rule, coupon_discount, discount_amount, coupon_storeid, coupon_for, coupon_picture from mycoupon where pid>0 ";					
						$sql2 = "SELECT pid,mid, coupon_no, using_flag, using_date, coupon_id ,coupon_name, coupon_type, coupon_description, DATE_FORMAT(coupon_startdate, '%Y-%m-%d') as coupon_startdate,DATE_FORMAT(coupon_enddate, '%Y-%m-%d') as coupon_enddate, coupon_status, coupon_rule, coupon_discount, discount_amount, coupon_storeid, coupon_for, coupon_picture,mycoupon_created_at,mycoupon_trash,b.sid,b.store_name,b.shopping_area,b.store_address,b.store_status from mycoupon as a INNER join (select sid,store_name,shopping_area,store_address,store_status,store_trash from store) as b on a.coupon_storeid=b.sid where a.mycoupon_trash=0 and b.store_trash=0 and a.pid>0 and coupon_type=8 ";					
						if ($mid != "") {	
							$sql2 = $sql2." and mid=".$mid."";
						}
						//if ($shopping_area != "") {	
						//	$sql2 = $sql2." and coupon_for=".$shopping_area."";
						//}					
						if ($using_flag != "") {	
