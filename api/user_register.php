...
function SendCURL($membername,$memberid,$memberpwd){
	try {
		...
		if ($obj["status"] == "error") {
		  ...
		  $result_noti = curl_error($crl); //die();
		} else {
		...
		try {
			...
			if ($result2 = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result2) > 0){
					...
					// register
					$sql="INSERT INTO member (member_id,member_pwd,member_name,member_type,recommend_code,member_status) VALUES ('$member_id', '$member_pwd', '$member_name',$member_type,'$recommend_code','0');";
					...
					if ($member_type == "1") {  //一般會員發 註冊平台禮
						...
						$sql2 = "SELECT * FROM coupon where coupon_trash=0 and coupon_type=6 and coupon_enddate > NOW()";   //and coupon_storeid='".$shoppingarea."'"
						...
						if ($recommend_code != ""){
							...
							// get 加入店家會員禮
							$sql6 = "SELECT * FROM coupon where coupon_trash=0 and coupon_type=3 and coupon_storeid='".$sid."' and coupon_enddate > NOW()";
