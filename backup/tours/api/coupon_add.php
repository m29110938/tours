...
$coupon_for = isset($_POST['coupon_for']) ? $_POST['coupon_for'] : '0';
	...
	if (($member_id != '') && ($member_pwd != '') && ($coupon_id != '') && ($coupon_name != '')) {
		//check 帳號/密碼
		$host = 'localhost';
		$user = 'tours_user';
		$passwd = 'tours0115';
		...
		try {
			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					$sql=$sql."('$coupon_id', '$coupon_name', $coupon_type, '$coupon_description', '$coupon_startdate', '$coupon_enddate', $coupon_status, $coupon_rule, $coupon_discount, $discount_amount, '$coupon_storeid', $coupon_for, NOW(), $mid);";
