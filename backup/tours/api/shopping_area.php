			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					try {
						...
						$sql2 = "SELECT `aid`, `shopping_area`, `contact_name`, `contact_phone` from shopping_area where shoppingarea_trash=0 ";					
						$sql2 = $sql2." order by aid";
						...
						if ($result2 = mysqli_query($link, $sql2)){
							if (mysqli_num_rows($result2) > 0){
								...
								$data["status"]="false";
								$data["code"]="0x0201";
								$data["responseMessage"]="This is no shopping area data!";									
							}
						}else{
