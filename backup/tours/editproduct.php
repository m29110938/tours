...
if (($_SESSION['authority']=="3")||($_SESSION['authority']=="4")){
	...
	$sql = "SELECT * FROM product where product_trash=0 and rid=$tid ";
	//echo $sql;
	//exit;
...
<body id="page-top">
  ...
  <div id="wrapper">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
			...
			<form action="productedit.php" name="frm3" id="frm3" method="post">
			<input type="hidden" name="act" id="act"  value="<?php echo $act;?>"/>		
			<input type="hidden" name="tid" id="tid"  value="<?php echo $tid;?>"/>		
			<input type="hidden" name="Dfilename" id="Dfilename"  value="<?php echo $product_picture;?>"/>
			
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">修改商品資料</h6>
                </div>
                <div class="card-body">
 					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<input type="text" name="product_no" id="product_no" class="form-control" value="<?php echo $product_no; ?>">
						  </div>
						</div>
					  </div>				
					  <div class="form-group">
						  ...
						  <div class="col-auto">
								<select name="product_type" id="product_type" class="form-control custom-select">
								  ...
								  <?php
									...
									$sql2 = "select pid,producttype_name from producttype where producttype_trash=0 order by producttype_name asc";
									
									if ($result2 = mysqli_query($link, $sql2)){
										if (mysqli_num_rows($result2) > 0){
											$selectedflag = "";
											while($row2 = mysqli_fetch_array($result2)){
												if ($product_type == strval($row2['pid'])) {
					  ...
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<input type="text" name="product_bonus" id="product_bonus" class="form-control" value="<?php echo $product_bonus; ?>"/>
						  </div>
						</div>
					  </div>
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<input type="text" name="product_stock" id="product_stock" class="form-control" value="<?php echo $product_stock; ?>"/>
						  </div>
						</div>
					  </div>
				...
				<tr >
					...
					<?php if ($product_picture != "")  { ?>
							<img src="<?php echo $product_picture;?>" ><br/><br/>
							<button type="button" id="del_map" name="del_map" onClick="DelRecord('<?php echo $tid; ?>','<?php echo $product_name; ?>');" class="btn btn-danger ml-auto">刪除</button>
							
 				    <?php }else{ ?>
							<img src="images/default_product.png" ><br/><br/>
 				    <?php } ?>						  
					...
					<td width="40%">
					  ...
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<textarea id="product_description" name="product_description" rows="4" cols="50"><?php echo $product_description; ?></textarea>							
						  </div>
						</div>
					  </div>
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
								<select name="product_status" id="product_status" class="form-control custom-select">
								  ...
								  <option value="1" <?php if ($product_status == "1") echo " selected "; ?>>上架</option>
								  <option value="0" <?php if ($product_status == "0") echo " selected "; ?>>下架</option>
				...
				<form action="uploadproductpic.php" name="frm2" id="frm2" method="post" enctype="multipart/form-data">
	...
	function DelRecord(eid,ename)
	{
		if (ename != "") {
			if (confirm("確定要刪除這個圖片嗎"+'('+ename+')?')) {
				...
				//alert(eid);
				document.frm3.action="productpicdel.php";	
	...
	function Save_User(){
		...
		if (document.all.product_type.value == "0") {
			alert("請選擇商品分類!");
			document.all.product_type.focus();
			return false;
		}		
		...
		if (document.all.product_status.value == "") {
			alert("請選擇商品狀態!");
			document.all.product_status.focus();
			return false;
		}
