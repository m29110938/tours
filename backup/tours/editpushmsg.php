...
if (($_SESSION['authority']=="3")||($_SESSION['authority']=="4")){
	...
	$sql = "SELECT * FROM pushmsg where pushmsg_trash=0 and pid=$tid";
	//echo $sql;
	//exit;
	//push_date,push_message,member_type,pushmsg_status
	...
	if ($result = mysqli_query($link, $sql)){
		if (mysqli_num_rows($result) > 0){
			while($row = mysqli_fetch_array($result)){
				...
				$pushmsg_status = $row['pushmsg_status'];
			}
		mysqli_close($link);
		}else{
				$push_date = "";
...
<!-- //push_date,push_message,member_type,pushmsg_status -->
			...
			<form action="pushmsgedit.php" name="frm3" id="frm3" method="post">
                ...
                <div class="card-body">
					  ...
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							  <input class="text-input small-input" type="date" name="push_date" id="push_date" value="<?php echo date('Y-m-d', strtotime($push_date));?>" />
						  </div>
						</div>
					  </div>					
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<textarea id="push_message" name="push_message" rows="4" cols="50"><?php echo $push_message;?></textarea>	
						  </div>
						</div>
					  </div>
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
								<select name="member_type" id="member_type" class="form-control custom-select">
								  <option value="0" <?php if ($member_type=="0") echo "selected"; ?>>--全部會員--</option>
	...
	function Save_User(){
		...
		if (!checkString(document.all.push_message,"請輸入推播訊息!"))	return false;
	...
	function GoAccount()
	{
			self.location.replace("pushmsg.php");
	}
	function Logout(){
		//alert("登出系統!");
