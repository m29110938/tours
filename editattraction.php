...
include 'phpqrcode/qrlib.php';
	...
	$sql = "SELECT * FROM attraction where store_trash=0 and sid=$tid";
	//echo $sql;
	//exit;
	...
	if ($result = mysqli_query($link, $sql)){
		if (mysqli_num_rows($result) > 0){
			...
			//$text = "https://coupon.jotangi.net:9443/app-attraction.php?tid=".$store_id;
			$text = "https://tripspot.jotangi.net/app-attraction.php?tid=".$store_id;
			//$text = "http://192.168.2.102/tours/app-attraction.php?tid=".$store_id;
		//mysqli_close($link);
		}else{
				$store_id = "";
				$store_type = "";
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
			...
			<form action="attractionedit.php" name="frm3" id="frm3" method="post">
			<input type="hidden" name="act" id="act"  value="<?php echo $act;?>"/>		
			<input type="hidden" name="tid" id="tid"  value="<?php echo $tid;?>"/>				  
			<input type="hidden" name="Dfilename" id="Dfilename"  value="<?php echo $store_picture;?>"/>
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">編輯景點資料</h6>
                </div>
                <div class="card-body">
				...
				<tr >
					...
					<td width="40%" valign="top">
						<div class="form-group">
						<label class="form-label">景點QRCODE:</label>
						<div class="row align-items-center">
						  <div class="col-auto">
							<?php
							try {
				...
				<form action="uploadattractionpic.php" name="frm2" id="frm2" method="post" enctype="multipart/form-data">
...
?>	
	...
	function DelRecord(eid,ename)
	{
		if (ename != "") {
			if (confirm("確定要刪除這個圖片嗎"+'('+ename+')?')) {
				...
				//alert(eid);
				document.frm3.action="attractionpicdel.php";	
	...
	function Save_User(){
		...
		if (!checkString(document.all.store_latitude,"請輸入景點緯度!"))	return false;
		if (!checkString(document.all.store_longitude,"請輸入景點精度!"))	return false;
