...
if ($_SESSION['authority']=="3"){
	...
	$sql = "SELECT * FROM push where push_trash=0 and pid=$tid";
	//echo $sql;
	//exit;
	//push_date,push_body,member_type,push_status
	...
	if ($result = mysqli_query($link, $sql)){
        ...
        $push_memberid = $row['push_memberid'];
...
<!-- //push_date,push_body,member_type,push_status -->
			...
			<form action="pushedit.php" name="frm3" id="frm3" method="post">
			<input type="hidden" name="act" id="act"  value="<?php echo $act;?>"/>		
			<input type="hidden" name="tid" id="tid"  value="<?php echo $tid;?>"/>		
			<input type="hidden" name="push_no" id="push_no"  value="<?php echo $push_no;?>"/>				  
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">編輯推播訊息</h6>
                </div>
                <div class="card-body">
					  ...
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							  <input class="text-input small-input" type="date" name="push_date" id="push_date" value="<?php echo date('Y-m-d', strtotime($push_date));?>"  min="<?=$today?>"/>
						  </div>
						</div>
					  </div>		
            <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<input type="text" id="push_title" name="push_title"  maxlength=50 value=<?=$push_title?>>
						  </div>
						</div>
					  </div>				
					  <div class="form-group">
						...
						<div class="row align-items-center">
						  <div class="col-auto">
							<textarea id="push_body" name="push_body" rows="4" cols="50"><?php echo $push_body;?></textarea>	
						  </div>
						</div>
					  </div>
					  <div class="form-group">
              ...
              <label>全選</label>
              <br> -->
              <input type="hidden" id="members" name="members" class="members" value="<?=$push_memberid?>">
              <select id="member" class="member" name="member" multiple style="width: 500px" data-placeholder="選擇會員">
                ...
                if ($push_storeid!="0"){
                  ...
                  $sql = $sql." where c.sid=".$push_storeid." and b.membercard_trash = 0 and a.member_trash = 0 and notificationToken is not null";
                }else{
                  $sql = $sql." where a.member_trash = 0 and notificationToken is not null";
                }
	...
	function Save_User(){
		...
		if (!checkString(document.all.push_body,"請輸入推播訊息!"))	return false;
  ...
  if ($mids[0] != 0){
    ...
    for ($i=0 ; $i<$mids_count ; $i++){
        echo "<script>$('.member .".$mids[$i]."').prop('selected', true); </script>";
    }
  }else{
    echo "<script>$('.member #sa').prop('selected', true); </script>";
    echo "<script>$('.member #m').prop('disabled', true); </script>";
  }
  echo "<script>$('.member').trigger('chosen:updated');</script>";
  ?>
  <script>
  check_status();
  function check_status(){
    ...
    console.log(status);
    if (status == 1){
			document.getElementById("push_title").disabled="disabled";
			document.getElementById("push_body").disabled="disabled";
			document.getElementById("push_date").disabled="disabled";
