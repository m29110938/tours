	...
	if ($result = mysqli_query($link, $sql)){
		if (mysqli_num_rows($result) > 0){
			...
			while($row = mysqli_fetch_array($result)){
				...
				$sql2 = $sql2." WHERE a.coupon_type in (2) and a.coupon_storeid > 0 and a.coupon_number = -9999 and a.coupon_status = 1 and a.coupon_trash = 0 and DATE(a.coupon_issue_startdate) <= DATE('".$sdate."') and DATE(a.coupon_issue_enddate) >= DATE('".$sdate."') and b.member_id='$mid'";
				...
				// 平台生日禮
				$sql2 = "SELECT * FROM `coupon` as a ";
				$sql2 = $sql2." WHERE a.coupon_type in (5) and a.coupon_storeid > 0 and a.coupon_number = -9999 and a.coupon_status = 1 and a.coupon_trash = 0 and DATE(a.coupon_issue_startdate) <= DATE('".$sdate."') and DATE(a.coupon_issue_enddate) >= DATE('".$sdate."')";
				if ($result2 = mysqli_query($link, $sql2)) {
					if (mysqli_num_rows($result2) > 0) {
						while ($row2 = mysqli_fetch_array($result2)) {
							$push_title = "生日禮來囉!";
