...
if (($sid != "")&&($mid != ""))  {
	...
	$amount1  = mysqli_real_escape_string($link,$amount1);
	...
	$sql=$sql." ('$orderid',NOW(),$sid,$tour_guide,$mid,$amount1,'',0,$pay_type,$amount1,0,0,1);";
...
$uuid = $orderid; //isset($_POST['uuid']) ? $_POST['uuid'] : '';
//$amount1 = isset($_POST['amount1']) ? $_POST['amount1'] : '';
if($uuid=='')
{
	//$url = "Location: index";
...
<body onload='Submit();'>
          ...
          <div class="container">
            <div class="row">
                <div class='row justify-content-center'>
					<form role="form"  name='frm1' id='frm1' method="POST" action="./allpay/request.php">
					...
					<input type="hidden" class="ctrl-input" name="amount1" value="<?php echo $amount1; ?>">			
	...
	<p class='text-center'><br>Copyright &copy;  2021 Jotangi Technology Co., Ltd.<br> All Right Reserved.</p>
    </div>
    <!-- /#wrapper -->
...
function Submit2()
{
	//document.location.href="hilifebuy";	
	document.location.href="https://coupon.jotangi.net:9443/reg/01/register.php";
}
function Count1(id)
{
	var num = document.getElementById('creditnumber1').value;
	if(num.length>=4)
	{
		//alert("4");
		document.getElementById('creditnumber2').focus();
	}
	if (num.length==0){alert("請輸入完整卡號。");}
}
function Count2(id)
{
	var num = document.getElementById('creditnumber2').value;
	if(num.length>=4)
	{
		//alert("4");
		document.getElementById('creditnumber3').focus();
	}
	if (num.length==0){alert("請輸入完整卡號。");}
}
function Count3(id)
{
	var num = document.getElementById('creditnumber3').value;
	if(num.length>=4)
	{
		//alert("4");
		document.getElementById('creditnumber4').focus();
	}
	if (num.length==0){alert("請輸入完整卡號。");}
}
