...
<body id="page-top">
      ...
      <li class="nav-item active">
		...
		<div id="collapseUtilities1" class="collapse show" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
		  <div class="bg-white py-2 collapse-inner rounded">
			...
			<a class="collapse-item active" href="ecorder.php">商城訂單管理</a>
		  </div>
		</div>			  
      </li> 
	  <?php } ?>
      ...
      <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
          ...
          <button id="sidebarToggleTop" class="btn btn-link d-md-none rounded-circle mr-3">
			...
			<h1 class="h5 mb-0 text-gray-800">商品管理</h1>
		...
		<?php
			...
			$staus = isset($_POST['product_status']) ? $_POST['product_status'] : '';
			...
			$producttype  = mysqli_real_escape_string($link,$producttype);
            ...
            <div class="col-lg-12 mb-4">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
				  ...
				  <div class="card-body">
				    <form action="product.php" method="Post" name='frm1' id='frm1' class="card">
					...
					<div class="row">
						...
						<div class="col-md-6 col-lg-2">
							<div class="form-group">
								...
								<select name="product_status" id="product_status" id="select-mapname" class="form-control custom-select">
								  <option value="">--全選--</option>
								  <option value="1" <?php if ($staus=="1") echo "selected"; ?>>上架</option>
								  <option value="2" <?php if ($staus=="2") echo "selected"; ?>>下架</option>
								</select>
							</div>
						</div>
						<div class="col-md-6 col-lg-2">
						  <div class="form-group">
							...
							<div class="row align-items-center">
							  <div class="col-auto">
								<input type="text" id="product_id" name="product_id" class="form-control w-12" value="<?php echo $productid; ?>">
							  </div>
							</div>
						  </div>	
						</div>						
						<div class="col-md-6 col-lg-2">
							<div class="form-group">
								...
								<select name="product_type" id="product_type" class="form-control custom-select">
								  <option value="">--全選--</option>
								  <?php
									$sql2 = "select product_type,producttype_name from producttype where producttype_trash=0 order by product_type,producttype_name asc";
									...
									if ($result2 = mysqli_query($link, $sql2)){
										if (mysqli_num_rows($result2) > 0){
											...
											while($row2 = mysqli_fetch_array($result2)){
												...
												echo "<option value='".$row2['product_type']."' ".$selectedflag." >".$row2['producttype_name']."</option>";
											}
										}
									}								
								  ?>
						...
						<div class="col-md-6 col-lg-2">
						  <div class="form-group">
							...
							<div class="row align-items-center">
							  <div class="col-auto">
								<input type="text" id="product_name" name="product_name" class="form-control w-12" value="<?php echo $productname; ?>">
							  </div>
							</div>
						  </div>	
						</div>
						...
						<div class="col-md-6 col-lg-2">
						  <div class="form-group">
							  ...
							  <div class="text-center">
								<button type="button" class='btn btn-info ml-auto' onclick='SubmitF();'>搜尋</button> &nbsp;<?php if ($_SESSION['authority']=="1"){ ?><button type="button" class="btn btn-success ml-auto" onclick='GoAddUser()'>新增</button> &nbsp;<!--<button type="submit" class="btn btn-primary ml-auto">匯入</button>--> <?php } ?>
							  </div>							
						  </div>
						</div>						
					</div>					
            ...
            <div class="card-body">
              <div class="table-responsive">
				...
				//if ($act == 'Qry') {
					...
					$sql = "SELECT a.*,b.product_type,b.producttype_name FROM product as a ";
					$sql = $sql." inner join ( select pid,product_type,producttype_name from producttype) as b ON a.pid= b.pid ";
					
					$sql = $sql." where a.product_trash=0 ";
					
					if ($staus != "") {	
						$sql = $sql." and product_status='".$staus."'";
					}					

					if (trim($productid) != "") {	
						$sql = $sql." and product_no like '%".trim($productid)."%'";
					}
					if (trim($productname) != "") {	
						$sql = $sql." and product_name like '%".trim($productname)."%'";
					}	
					if ($producttype != "") {	
						$sql = $sql." and b.product_type = '".$producttype."'";
					}					
			
					$sql = $sql." order by b.product_type,a.product_no ";
					//echo $sql;
					//exit;
					...
					if ($result = mysqli_query($link, $sql)){
						if (mysqli_num_rows($result) > 0){
							...
							while($row = mysqli_fetch_array($result)){
									...
									echo "    <td>".$row['producttype_name']."</td>";
								
									echo "    <td>".$row['product_name']."</td>";
									echo "    <td align=right>".$row['product_price']."</td>";
							
									echo "    <td align=right>".$row['product_stock']."</td>";
									echo "    <td>".date('Y-m-d', strtotime($row['product_created_at']))."</td>";
  ...
  <SCRIPT LANGUAGE=javascript>
	...
	function ExportLog()
	{
		document.all.downloadlog.src="product.php";
	}	
	function GoAddUser()
	{	
		document.getElementById("act").value = 'Add';
		document.frm1.action="addproduct.php";
	...
	{	
		...
		//alert(id);
		document.frm1.action="editproduct.php";	
		document.frm1.submit();
	}
	function GoDel(id)
	{	
		if (confirm('確定要刪除這筆資料嗎?')) {
			...
			document.getElementById("tid").value = id;
			//alert(id);
			document.frm1.action="productdel.php";	
			document.frm1.submit();
		}
	}	
	function Logout(){
