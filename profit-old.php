      ...
      <li class="nav-item  active">
		...
		<div id="collapseUtilities2" class="collapse show" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
		  <div class="bg-white py-2 collapse-inner rounded">
			...
			<a class="collapse-item active" href="profit.php">分潤報表(對帳)</a>
		  </div>
		</div>			  
      </li>
	  <?php if ($_SESSION['authority']!="4"){  ?>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
		...
		<?php
			...
			$bonus_mode = isset($_POST['bonus_mode']) ? $_POST['bonus_mode'] : '';
			$bonus_mode  = mysqli_real_escape_string($link,$bonus_mode);
						...
						<div class="col-md-6 col-lg-2">
							<div class="form-group">
								...
								<select name="bonus_mode" id="bonus_mode" class="form-control custom-select">
								  ...
								  <option value="1" <?php if ($bonus_mode=="1") echo "selected"; ?>>一般消費模式</option>
								  <option value="2" <?php if ($bonus_mode=="2") echo "selected"; ?>>活動消費模式</option>
								</select>
							</div>
						</div>
            ...
            <div class="card-body">
              <div class="table-responsive">
				...
				//if ($act == 'Qry') {
					...
					$sql = "SELECT a.*,b.store_name FROM profit_share as a ";
					...
					if ($bonus_mode != "") {	
						$sql = $sql." and a.bonus_mode=".$bonus_mode."";
					}					
					//if ($paytype != "") {	
					...
					if ($SDate != "") {	
						$sql = $sql." and a.profit_date >= '".$SDate." 00:00:00'";
					}
					if ($EDate != "") {	
						$sql = $sql." and a.profit_date <= '".$EDate." 23:59:59'";
					}			
					$sql = $sql." order by a.store_id,a.profit_date,order_no ";
					//echo $sql;
					//exit;
					$idx = 0;
					...
					if ($result = mysqli_query($link, $sql)){
						if (mysqli_num_rows($result) > 0){
							...
							while($row = mysqli_fetch_array($result)){
									...
									echo "    <td>".date('Y-m-d', strtotime($row['profit_date']))."</td>";
									echo "    <td>".$row['order_amount']."</td>";
									$sum1 = $sum1 + $row['order_amount'];
									echo "    <td>".$row['order_pay']."</td>";
									$sum2 = $sum2 + $row['order_pay'];
									$order_pay = $row['order_pay'];
									echo "    <td>".$row['bonus_name1']."</td>";
									echo "    <td>".floor($order_pay * $row['sys_rate1'] )."</td>";
									$sum3 = $sum3 + floor($order_pay * $row['sys_rate1'] );
									echo "    <td>".floor($order_pay * $row['marketing_rate1'] )."</td>";
									$sum4 = $sum4 + floor($order_pay * $row['marketing_rate1'] );
									echo "    <td>".$row['bonus_name2']."</td>";
									echo "    <td>".floor($order_pay * $row['sys_rate2'] )."</td>";
									$sum5 = $sum5 + floor($order_pay * $row['sys_rate2'] );
									echo "    <td>".floor($order_pay * $row['marketing_rate2'] )."</td>";
									$sum6 = $sum6 + floor($order_pay * $row['marketing_rate2'] );
									if ($row['bonus_mode'] == 1){
										echo "    <td>".floor($order_pay * $row['user_rate'] )."</td>";
										$sum7 = $sum7 + floor($order_pay * $row['user_rate'] );
									}
									if ($row['bonus_mode'] == 2){
										echo "    <td>".floor($order_pay * $row['event_rate'] )."</td>";
										$sum7 = $sum7 + floor($order_pay * $row['event_rate'] );
									}									
									echo "    <td>".floor($order_pay * $row['groupmode_rate'] )."</td>";
									$sum8 = $sum8 + floor($order_pay * $row['groupmode_rate'] );
									echo "    <td>".floor($order_pay * $row['hotelmode_rate'] )."</td>";
									$sum9 = $sum9 + floor($order_pay * $row['hotelmode_rate'] );
