<?php
	...
	// 要改時間
	$sql = $sql." WHERE date_add(DATE(bonus_end_date),interval 1 day) = DATE('".$year.'-'.$today."') and member_id = 3215";
		...
		if (mysqli_num_rows($result) > 0){
			...
			while($row = mysqli_fetch_array($result)){
				...
				if (($endBonusGet - $bonusPoint) > 0 ){
					...
					$sql4 = $sql4." ('$order_no',129,1,'$mid',-1,0,-1,0,1,0,0,1,'$bonusEndDate','$bonusEndDate','$bonusEndDate',-'$addBonus');";
